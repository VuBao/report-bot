...
Bot đọc toàn bộ trường cần thiết từ ảnh gốc ở lượt đầu. Nếu địa chỉ đọc được đầy đủ, confidence đạt ngưỡng và model không đánh dấu còn nghi vấn, bot dùng ngay kết quả lượt đầu và không gọi OCR lần nữa. Chỉ khi địa chỉ thiếu, confidence thấp, dữ liệu mặt sau chưa đầy đủ hoặc model đặt `address_review_required=true`, bot mới chạy lượt hai: tự crop vùng địa chỉ theo bố cục mặt trước/mặt sau của thẻ, phóng lớn 3 lần, tăng tương phản và độ nét nhẹ rồi đọc lại địa chỉ. Khi lượt hai được kích hoạt, kết quả crop thay thế phần địa chỉ còn nghi vấn và phải đạt ngưỡng confidence; nếu crop vẫn không đủ chắc chắn, bot dừng và yêu cầu ảnh rõ hơn. Sau khi vượt qua validation, bot đối chiếu họ tên trong payload rồi gửi preview. Chỉ sau tin nhắn `XAC NHAN` từ chính người gửi ảnh, bot mới ghi form: `B2`, ngày tạo tại `E2`, `B3`, `B4`, `B5`, `E5`, và báo cáo dịch tại `B31`, `B33`. Chỉ vùng ngày sửa `E2:F2` và hai vùng báo cáo `B31:F31`, `B33:F33` được tô màu hoàn tất; các trường thẻ và màu tab giữ nguyên format của template. Nếu chưa có file của công ty, bot copy file `COPY` vào Drive folder để tạo form mới. Gửi `HUY` để bỏ yêu cầu. Ảnh thẻ không được lưu sau khi đọc.
